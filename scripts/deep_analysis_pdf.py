...
def build_pdf():
    doc = SimpleDocTemplate(
        '/Users/serhiykorenyev/Desktop/vs code/widget cs /Analisi /BetonWin_5Month_Deep_Analysis.pdf',
        pagesize=A4,
    ...
    # ══════════════════════════════════════════════════════════
    story.append(Paragraph('2. Analisi dei Cluster (Argomenti Principali)', styles['H1']))
    story.append(hr())

    for i, c in enumerate(CLUSTERS):
        story.append(Paragraph(f'2.{i+1} {c["topic"]} — {c["pct"]}', styles['H2']))
        story.append(Paragraph(f'<b>Volume totale (5 mesi):</b> ~{c["total"]:,} | <b>Media mensile:</b> ~{c["avg"]:,}', styles['Body']))
    ...
    # ══════════════════════════════════════════════════════════
    story.append(Paragraph('3. Analisi Utenti Univoci e Rischio Churn', styles['H1']))
    story.append(hr())
    story.append(Paragraph(
    ...
    # 3.1 Overview
    story.append(Paragraph('3.1 Overview Utenti', styles['H2']))
    user_overview = [
        ['Metrica', 'Valore'],
    ...
    story.append(spacer(4))
    story.append(Paragraph(
        '<b>Il 28.3% degli utenti genera circa il 51.5% di tutti i ticket.</b> Questi utenti ricorrenti sono il target primario per ridurre il volume di supporto.',
        styles['Body']
    ))
    story.append(spacer(6))

    # 3.2 Distribution
    story.append(Paragraph('3.2 Distribuzione Utenti Multi-Ticket', styles['H2']))
    dist_data = [
    ...
    story.append(Paragraph('3.3 Cluster di Frustrazione — Utenti che Riaprono sullo Stesso Argomento', styles['H2']))
    story.append(Paragraph(
    ...
    story.append(Paragraph('3.3.1 CRITICO — "How to Withdraw" (38 utenti, 2.7 ticket ciascuno)', styles['H3']))
    story.append(Paragraph(
    ...
    story.append(Paragraph('3.3.2 CRITICO — "Deposit Not Credited" (27 utenti, 2.8 ticket ciascuno)', styles['H3']))
    story.append(Paragraph(
    ...
    story.append(Paragraph('3.3.3 ALTO — "Technical / App / Website" (51 utenti, 2.5 ticket ciascuno)', styles['H3']))
    story.append(Paragraph(
    ...
    # 3.4 Top 10 users
    story.append(Paragraph('3.4 Top 10 Utenti ad Alto Rischio', styles['H2']))
    top_users = [
    ...
    # 3.5 Persistence
    story.append(Paragraph('3.5 Persistenza Utenti Ricorrenti nel Tempo', styles['H2']))
    persist_data = [
        ['Mesi di Attivita', 'Utenti Ricorrenti', '% dei Ricorrenti', 'Interpretazione'],
        ...
        ['5 mesi (tutti)', '~42', '1.0%', 'MASSIMO RISCHIO — contattano supporto ogni mese'],
    ]
    ...
    story.append(Paragraph('3.6 Raccomandazioni di Ritenzione per Cluster', styles['H2']))
    ...
    ret1 = [
        ...
        ['KB dedicata', 'Pagina "Perche non posso prelevare" con checklist visuale', '3 giorni'],
        ['Widget AI + GR8', 'Check via API se ha bonus attivo, spiegare proattivamente', '2 settimane'],
    ]
    ...
    ret2 = [
        ...
        ['KB tempi', 'Tempi per metodo (MercadoPago: 5min, BankTransfer: 2-24h)', '3 giorni'],
        ['Widget AI + GR8', 'Integrare paymentTransactionV2 per status deposito immediato', '2 settimane'],
    ]
    story.append(make_table(ret2, col_widths=[W*0.22, W*0.52, W*0.15], header_color=BRAND_AMBER))
    story.append(spacer(4))

    # 5-month users
    story.append(Paragraph('Per utenti 5-mesi (42 utenti)', styles['H3']))
    ret3 = [
        ['Azione', 'Dettaglio', 'Timeline'],
    ...
    # P0
    story.append(Paragraph('P0 — CRITICAL (Immediate — questa settimana)', styles['H2']))
    p0_data = [
        ...
        ['P0-1', 'Creare sezione KB completa "Bank Transfer / Comprobante": dettagli bancari, requisiti comprobante, upload, tempi per paese',
         'Bank Transfer (12.3%)', '-40% ticket su topic'],
    ...
    story.append(Paragraph(
        ...
        'Cross-referenza con dati real-time pagamenti GR8 (paymentOrder + paymentTransactionV2).</i>',
        styles['BodySmall']
    ))

    # Build
    doc.build(story)
    print('PDF generated: BetonWin_5Month_Deep_Analysis.pdf')
