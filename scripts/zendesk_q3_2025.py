...
MONTHS = [
    {
        ...
        'short': 'Jul 2025',
        'weeks': [
            ('2025-07-01T00:00:00Z', '2025-07-08T00:00:00Z'),
            ('2025-07-08T00:00:00Z', '2025-07-15T00:00:00Z'),
            ('2025-07-15T00:00:00Z', '2025-07-22T00:00:00Z'),
            ('2025-07-22T00:00:00Z', '2025-08-01T00:00:00Z'),
        ],
        'days': 31,
    },
    {
        ...
        'weeks': [
            ...
            ('2025-08-22T00:00:00Z', '2025-09-01T00:00:00Z'),
        ],
        'days': 31,
    },
    {
        ...
        'short': 'Sep 2025',
        'weeks': [
            ('2025-09-01T00:00:00Z', '2025-09-08T00:00:00Z'),
            ('2025-09-08T00:00:00Z', '2025-09-15T00:00:00Z'),
            ('2025-09-15T00:00:00Z', '2025-09-22T00:00:00Z'),
            ('2025-09-22T00:00:00Z', '2025-10-01T00:00:00Z'),
        ],
        'days': 30,
    },
]
...
doc = SimpleDocTemplate(
    'zendesk_q3_2025_report.pdf', pagesize=A4,
...
story.append(Paragraph('BetonWin — Q3 2025 Support Analysis', title_s))
story.append(Paragraph(f'July - September 2025 | {grand_total} tickets | {grand_comments} comments', sub_s))
...
mc_rows = [['Category', 'Jul 2025', 'Aug 2025', 'Sep 2025', 'Total', 'Trend']]
...
res_rows = [['Metric', 'Jul 2025', 'Aug 2025', 'Sep 2025']]
...
# Calculate trends
jul_d = all_month_data['July 2025']
sep_d = all_month_data['September 2025']
aug_d = all_month_data['August 2025']

vol_change = round((sep_d['total'] - jul_d['total']) / max(jul_d['total'], 1) * 100)

# Find fastest growing topic
topic_growth = {}
for topic in set(jul_d['topic_counts'].keys()) | set(sep_d['topic_counts'].keys()):
    jul_v = jul_d['topic_counts'].get(topic, 0)
    sep_v = sep_d['topic_counts'].get(topic, 0)
    if jul_v > 10:
        topic_growth[topic] = round((sep_v - jul_v) / jul_v * 100)
growing = sorted(topic_growth.items(), key=lambda x: -x[1])
shrinking = sorted(topic_growth.items(), key=lambda x: x[1])

insights = [
    f'<b>VOLUME TREND:</b> {vol_change:+d}% change from July ({jul_d["total"]}) to September ({sep_d["total"]}). '
    f'August had {aug_d["total"]} tickets.',

    f'<b>TOTAL Q3:</b> {grand_total} tickets across 3 months = ~{round(grand_total/92)} tickets/day average.',
]
...
insights.append(
    f'<b>AUTOMATION POTENTIAL:</b> {auto_high_total} tickets ({auto_pct}%) across Q3 could be fully automated '
    f'by the AI widget (greetings + deposits + bonus FAQ + account info).'
)
insights.append(
    f'<b>ESTIMATED Q3 SAVINGS:</b> At $3-5/ticket, automating {auto_high_total} tickets would save '
...
story.append(Paragraph(
    ...
    f'BetonWin Zendesk | Q3 2025 | {grand_total} tickets | {grand_comments} comments',
    cap_s))

doc.build(story)
print(f'\n{"=" * 60}')
print(f'  PDF saved: zendesk_q3_2025_report.pdf')
