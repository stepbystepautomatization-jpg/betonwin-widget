...
def build_pdf():
    doc = SimpleDocTemplate(
        '/Users/serhiykorenyev/Desktop/vs code/widget cs /Analisi /BetonWin_User_Recovery_Playbook.pdf',
        pagesize=A4,
    ...
    story.append(Paragraph('Analisi Dettagliata Utenti &amp; Strategie di Recupero', styles['H1']))
    story.append(hr())
    ...
    # Cover key numbers
    cover_data = [
        ['Metrica Chiave', 'Valore', 'Significato'],
        ['Utenti totali', '14,888', 'Base utenti attiva nel supporto'],
        ['Utenti ricorrenti (2+ ticket)', '4,215 (28.3%)', 'Target primario per recovery'],
        ['Ticket da utenti ricorrenti', '~11,327 (51.5%)', 'Meta dei ticket da 28% degli utenti'],
        ['Utenti ad alto rischio (6+ ticket)', '149', 'Intervento immediato necessario'],
        ['Utenti cronici (3+ mesi)', '~42', 'Massimo rischio churn'],
        ['Costo stimato churn/utente', '$180-$420/anno', 'Basato su LTV medio iGaming LATAM'],
        ['Revenue a rischio (149 utenti)', '$26,820-$62,580/anno', 'Solo utenti 6+ ticket'],
    ]
    ...
    # ═══════════════════════════════════════════════
    story.append(Paragraph('1. Segmentazione Utenti — La Piramide del Rischio', styles['H1']))
    story.append(hr())
    ...
    # Pyramid table
    pyramid = [
        ['Livello', 'Utenti', '% Utenti', 'Ticket Generati', '% Ticket', 'Rischio', 'Azione'],
        ['CRITICO\n(11+ ticket)', '14', '0.09%', '~210', '0.95%', 'MASSIMO', 'Account manager dedicato'],
        ['ALTO\n(6-10 ticket)', '135', '0.91%', '~1,013', '4.60%', 'ALTO', 'Outreach proattivo'],
        ['MODERATO\n(3-5 ticket)', '1,406', '9.44%', '~4,920', '22.36%', 'MEDIO', 'Auto-escalation widget'],
        ['BASSO\n(2 ticket)', '2,660', '17.87%', '5,320', '24.18%', 'BASSO', 'KB improvement'],
        ['MINIMO\n(1 ticket)', '10,673', '71.69%', '10,673', '48.51%', 'MINIMO', 'Self-service standard'],
    ]
    story.append(make_table(pyramid, col_widths=[W*0.12, W*0.08, W*0.08, W*0.12, W*0.08, W*0.10, W*0.25], header_color=BRAND_RED))
    story.append(spacer(6))

    story.append(Paragraph('Insight Chiave sulla Piramide:', styles['H3']))
    insights = [
    ...
    # ═══════════════════════════════════════════════
    story.append(Paragraph('2. Personas Utente — Chi Sono i Nostri Utenti Frustrati', styles['H1']))
    story.append(hr())
    story.append(Paragraph(
    ...
    story.append(Paragraph('PERSONA 1: "Il Depositante Frustrato"', styles['H2']))
    story.append(Paragraph(
    ...
    p1_data = [
        ['Metrica', 'Valore'],
        ['Utenti identificati', '~58 (27 Deposit Not Credited + 31 Bank Transfer)'],
        ['Ticket totali generati', '~149 (75 + 74)'],
        ['Media ticket/utente', '2.6'],
        ['Tempo medio risoluzione', '3-5 giorni (troppo lungo)'],
        ['Mesi attivi', '1-2 mesi (burst poi abbandono)'],
    ...
    story.append(Paragraph('Il suo percorso tipico (User Journey):', styles['H3']))
    journey1 = [
        ['Giorno', 'Azione Utente', 'Cosa Prova', 'Cosa Dovremmo Fare'],
        ['Giorno 1', 'Deposita $10,000 ARS via bonifico', 'Fiducia, entusiasmo', 'Conferma immediata: "Deposito ricevuto, in elaborazione"'],
        ['Giorno 1 (+2h)', 'Non vede saldo. Apre ticket #1:\n"deposite y no aparece"', 'Preoccupazione', 'Widget: check paymentTransactionV2 via GR8 API, dare status in tempo reale'],
        ['Giorno 2', 'Invia comprobante via chat.\nTicket #2: "mande comprobante"', 'Frustrazione', 'Auto-risposta: "Comprobante ricevuto, saldo accreditato entro 4h lavorative"'],
        ['Giorno 3', 'Ticket #3: "ya son 3 dias\ny no me acreditaron"', 'Rabbia, sfiducia', 'Escalation automatica a agente senior + notifica push appena accreditato'],
        ['Giorno 5+', 'Saldo accreditato. Ma utente\ne gia andato su un competitor.', 'Disincanto', 'Follow-up: "Il tuo saldo e disponibile + bonus fedeltà di $500 ARS"'],
    ]
    story.append(make_table(journey1, col_widths=[W*0.10, W*0.26, W*0.18, W*0.36], header_color=BRAND_DARK))
    story.append(spacer(3))

    story.append(Paragraph('Strategia di Recupero Concreta:', styles['H3Green']))
    rec1 = [
    ...
    story.append(Paragraph('Template Email Win-Back (Depositante Frustrato):', styles['H3']))
    story.append(Paragraph(
    ...
    story.append(Paragraph('PERSONA 2: "Il Vincitore Bloccato"', styles['H2']))
    story.append(Paragraph(
    ...
    p2_data = [
        ['Metrica', 'Valore'],
        ['Utenti identificati', '~38 (same-topic How to Withdraw) + ~33 (KYC bloccati)'],
        ['Ticket totali generati', '~181 (103 withdrawal + 78 KYC)'],
    ...
    story.append(Paragraph('Il suo percorso tipico:', styles['H3']))
    journey2 = [
        ['Giorno', 'Azione Utente', 'Cosa Prova', 'Cosa Dovremmo Fare'],
        ['Giorno 1', 'Deposita $20,000 ARS.\nAccetta bonus 100%.', 'Entusiasmo', 'Pop-up chiaro: "Accettando il bonus, dovrai scommettere $700,000 ARS prima di prelevare"'],
        ['Giorno 3', 'Vince $74,000 ARS.\nProva a prelevare. BLOCCATO.', 'Confusione', 'Widget: "Hai un bonus attivo. Mancano $350,000 ARS di scommesse. Vuoi rinunciare al bonus?"'],
        ['Giorno 3', 'Ticket #1: "gane 74.000\ny no lo puedo retirar"', 'Frustrazione', 'Agente: spiega rollover con numeri esatti, offre opzione di rinuncia bonus'],
        ['Giorno 5', 'Ticket #2: "ya es segunda vez,\ncada vez no me deja"', 'Rabbia', 'Auto-escalation + link diretto alla pagina "Come prelevare - Checklist"'],
        ['Giorno 7+', 'Smette di giocare.\nVa su competitor.', 'Sfiducia totale', 'Follow-up: "Puoi prelevare rinunciando al bonus. Ecco come: [LINK]"'],
    ]
    ...
    story.append(Paragraph('Esempio Concreto — Calcolatrice Rollover:', styles['H3']))
    story.append(Paragraph(
        ...
        '<b>Significato pratico:</b> Deve scommettere $350,000 CLP in totale prima di poter prelevare<br/>'
    ...
    story.append(Paragraph('PERSONA 3: "Il Tech-Frustrato (Mobile)"', styles['H2']))
    story.append(Paragraph(
    ...
    p3_data = [
        ['Metrica', 'Valore'],
        ['Utenti identificati', '51 (same-topic Tech frustration)'],
        ['Ticket totali generati', '127'],
        ['Media ticket/utente', '2.5'],
        ['% problemi mobile', '72.2%'],
        ['% problemi browser', '18.0%'],
        ['Impatto revenue', 'TOTALE — utente bloccato non genera nulla'],
    ...
    story.append(Paragraph('Problemi Specifici Identificati dai Messaggi:', styles['H3']))
    tech_problems = [
        ['Problema', 'Frequenza', 'Causa Probabile', 'Soluzione Widget'],
        ['App non si apre', '~28%', 'Cache corrotta o browser vecchio', 'Guida: "Pulisci cache > Riapri browser > Se persiste, usa Chrome/Firefox"'],
        ['Errore al deposito', '~22%', 'Sessione scaduta o pop-up bloccati', '"Disattiva blocco pop-up nelle impostazioni del browser, poi riprova"'],
        ['Schermata bianca', '~18%', 'JavaScript disabilitato o ad-blocker', '"Disattiva ad-blocker > Attiva JavaScript > Ricarica la pagina"'],
        ['Non trova app store', '~15%', 'PWA non e app nativa', '"BetonWin e un sito web ottimizzato per mobile. Per salvarlo: Menu > Aggiungi a Home"'],
        ['Numero telefono errato', '~12%', 'Errore di registrazione', '"Per correggere il numero, contatta supporto con il tuo documento di identita"'],
        ['Lentezza estrema', '~5%', 'Connessione lenta + sito pesante', '"Prova con connessione Wi-Fi. Se persiste, usa la versione lite: [LINK]"'],
    ]
    story.append(make_table(tech_problems, col_widths=[W*0.16, W*0.10, W*0.26, W*0.38], header_color=BRAND_DARK))
    story.append(spacer(3))

    story.append(Paragraph('Strategia di Recupero:', styles['H3Green']))
    rec3 = [
        ...
        '&bull; <b>ENTRO 2 SETTIMANE:</b> Creare pagina di test diagnostico: l\'utente clicca un link e il sistema verifica browser, JavaScript, pop-up, connessione e restituisce "Il tuo dispositivo e compatibile" oppure "Problema trovato: [soluzione]".',
    ...
    story.append(Paragraph('PERSONA 4: "Il Giocatore Confuso dal Bonus"', styles['H2']))
    story.append(Paragraph(
    ...
    p4_data = [
        ['Metrica', 'Valore'],
        ['Utenti identificati', '~30 (Bonus) + ~34 (Casino) + ~21 (Balance) = ~85 potenziali'],
        ['Ticket totali generati', '~206 (79 + 80 + 47)'],
    ...
    story.append(Paragraph('Il Problema "Saldo 0.00" Spiegato:', styles['H3']))
    story.append(Paragraph(
    ...
    rec4 = [
        ...
        '&bull; <b>ENTRO 1 SETTIMANA:</b> Suggerire al team prodotto di mostrare il saldo TOTALE (reale + bonus) nella schermata principale, con un link "Dettagli saldo" che mostra la suddivisione.',
    ...
    story.append(Paragraph('PERSONA 5: "Il Lamentatore Cronico"', styles['H2']))
    story.append(Paragraph(
        '<b>Profilo:</b> Apre ticket multipli con accuse di frode, insulti, minacce. Volume basso (3 utenti same-topic, 11 ticket) '
        'ma impatto reputazionale ALTO — questi utenti lasciano review negative, post sui social, denunce. '
        'Crescita 4.5x in 5 mesi (da 8 a 36 ticket/mese di complaint).',
        styles['PersonaBox']
    ))
    story.append(spacer(3))

    story.append(Paragraph('Messaggi Reali — Livelli di Escalation:', styles['H3']))
    complaint_levels = [
        ['Livello', 'Esempio Messaggio', 'Stato Emotivo', 'Risposta Consigliata'],
        ['1 - Frustrazione', '"Quiero hacer un reclamo.\nSolo roban el dinero"', 'Arrabbiato ma\nrecuperabile', 'Scusarsi + spiegare + compensare'],
    ...
    story.append(Paragraph('Template Risposta Professionale (Livello 2 — Accusa Frode):', styles['H3']))
    story.append(Paragraph(
        'Hola [NOMBRE],<br/><br/>'
    ...
    story.append(Paragraph('PERSONA 6: "Il Chattatore Seriale"', styles['H2']))
    story.append(Paragraph(
    ...
    p6_data = [
        ['Metrica', 'Valore'],
        ['Utenti identificati', '~195 (same-topic Greeting) + top heavy-users'],
        ['Ticket totali generati', '~571 (solo greeting)'],
        ...
        ['Costo supporto/utente', '~$2-5 per "hola" = $74-$185 per il top user'],
        ['Rischio churn', 'BASSO — sono utenti fedeli, solo inefficienti'],
    ]
    story.append(make_table(p6_data, col_widths=[W*0.30, W*0.60], header_color=BRAND_GRAY))
    story.append(spacer(3))

    story.append(Paragraph('Esempio Reale — User #1 (29547546107410):', styles['H3']))
    story.append(Paragraph(
        ...
        '&bull; Frequenza: 1.3 ticket AL GIORNO per quasi un mese<br/>'
        '&bull; Pattern: apre chat, scrive poche parole, non ottiene risposta immediata, chiude, riapre<br/><br/>'
        '<b>Diagnosi:</b> Questo utente non sa come usare il sistema di supporto. Pensa che ogni messaggio '
    ...
    # ═══════════════════════════════════════════════
    story.append(Paragraph('3. Costo del Churn — Quanto Perdiamo', styles['H1']))
    story.append(hr())
    story.append(Paragraph(
    ...
    # Cost table
    cost_data = [
        ['Segmento', 'Utenti', 'LTV/Utente/Anno', 'Revenue Annua\na Rischio', 'Probabilita\nChurn', 'Perdita Attesa\n/Anno'],
        ['CRITICO (11+ ticket)', '14', '$300-$420', '$4,200-$5,880', '70%', '$2,940-$4,116'],
        ['ALTO (6-10 ticket)', '135', '$240-$360', '$32,400-$48,600', '50%', '$16,200-$24,300'],
        ['MODERATO (3-5 ticket)', '1,406', '$180-$300', '$253,080-$421,800', '30%', '$75,924-$126,540'],
        ['BASSO (2 ticket)', '2,660', '$180-$240', '$478,800-$638,400', '15%', '$71,820-$95,760'],
        ['TOTALE a rischio', '4,215', '—', '$768,480-$1,114,680', '—', '$166,884-$250,716'],
    ]
    ...
    # ROI of recovery
    story.append(Paragraph('ROI delle Strategie di Recupero:', styles['H2']))
    roi_data = [
        ...
        ['TOTALE', '$4,800-$9,500', '~277 utenti', '$58,380-$99,180', '614-2,066%'],
    ]
    story.append(make_table(roi_data, col_widths=[W*0.22, W*0.15, W*0.14, W*0.16, W*0.12], header_color=BRAND_GREEN))
    story.append(spacer(6))

    story.append(Paragraph(
        '<b>Investimento totale: $4,800-$9,500 | Revenue salvata: $58,380-$99,180/anno | ROI: 614-2,066%</b><br/>'
    ...
    # ═══════════════════════════════════════════════
    story.append(Paragraph('4. Case Study Dettagliati — I 5 Utenti Piu Critici', styles['H1']))
    story.append(hr())
    ...
    cs1 = [
        ['Campo', 'Dettaglio'],
        ['Ticket totali', '37 in 28 giorni (9 Oct - 6 Nov 2025)'],
        ['Frequenza', '1.3 ticket/giorno — il piu alto nella base utenti'],
        ['Topic', 'Tutti "Uncategorized" — messaggi brevi via chat'],
        ['Pattern', 'Apri-chiudi ripetuto. Non usa il sistema correttamente.'],
        ['Mesi attivi', '2 (Oct-Nov), poi scomparso — probabile churn'],
        ['Diagnosi', 'Utente LATAM nuovo, non capisce il ticketing.\nProbabilmente scrive "hola" e aspetta, poi riapre.'],
    ...
    story.append(Paragraph('Azione Specifica:', styles['H3Green']))
    story.append(Paragraph(
    ...
    cs2 = [
        ['Campo', 'Dettaglio'],
        ['Ticket totali', '9 in 8 giorni (16-24 Oct 2025)'],
        ...
        ['Problema probabile', 'Bloccato da bonus rollover o KYC incompleto.\nNessun agente ha risolto il problema root.'],
        ['Mesi attivi', '1 (solo Oct), poi scomparso — CHURN CONFERMATO'],
    ...
    story.append(Paragraph('Cosa e Andato Storto (Root Cause Analysis):', styles['H3Red']))
    story.append(Paragraph(
        ...
        '&bull; <b>Ticket 4-6 (19-21 Oct):</b> Frustrazione crescente. Agenti diversi ogni volta. Nessun contesto.<br/>'
    ...
    story.append(Paragraph('Come Prevenire in Futuro:', styles['H3Green']))
    story.append(Paragraph(
    ...
    cs3 = [
        ['Campo', 'Dettaglio'],
        ['Ticket totali', '12 in 21 giorni (3-24 Dec 2025)'],
        ...
        ['Problema probabile', 'Metodo di pagamento non disponibile (es. Cuenta Rut\nnon supportata) oppure errore tecnico ricorrente'],
        ['Mesi attivi', '1 (solo Dec), poi scomparso — MAI DEPOSITATO'],
        ['Impatto', 'Utente acquisito (CAC pagato: $45-$90) che non ha\nMAI convertito. Investimento perso al 100%.'],
    ]
    story.append(make_table(cs3, col_widths=[W*0.18, W*0.72], header_color=BRAND_AMBER))
    story.append(spacer(3))

    story.append(Paragraph('Recovery per Utenti "Mai Convertiti":', styles['H3Green']))
    story.append(Paragraph(
        '&bull; <b>Identificare:</b> Query Zendesk — utenti con 3+ ticket "How to Deposit" + 0 depositi su GR8 API<br/>'
        '&bull; <b>Diagnosi automatica:</b> Il widget dovrebbe chiedere: "Da quale paese stai accedendo?" e mostrare '
    ...
    cs4 = [
        ['Campo', 'Dettaglio'],
        ['Ticket totali', '7 in 18 ore (8 Oct 2025, 03:45 - 22:05)'],
        ['Topic', 'TUTTI "Bonus / Promotions"'],
        ['Pattern', 'Burst acuto: 7 ticket in 1 giorno. Crisi bonus.'],
        ['Problema probabile', 'Ha accettato bonus e non capisce le condizioni.\nProbabilmente ha vinto ma non puo prelevare\noppure saldo mostra 0.00.'],
        ['Urgenza', 'MASSIMA — utente in crisi emotiva, potenziale complaint'],
    ]
    ...
    # ═══════════════════════════════════════════════
    story.append(Paragraph('5. Piano di Recovery Completo — Timeline Operativa', styles['H1']))
    story.append(hr())

    # Week 1
    story.append(Paragraph('SETTIMANA 1 (11-17 Marzo 2026) — Quick Wins', styles['H2']))
    w1 = [
        ['Azione', 'Chi', 'Persona Impattata', 'Ticket/Mese Risparmiati', 'Costo'],
        ['KB: "Il mio deposito non appare"\n(timeline per metodo)', 'Content Team', 'Depositante Frustrato', '~70', '$0 (contenuto)'],
        ['KB: "Saldo 0.00 con bonus"\n(spiegazione prominente)', 'Content Team', 'Giocatore Confuso', '~85', '$0 (contenuto)'],
        ['KB: "Come prelevare - Checklist"\n(pre-requisiti visivi)', 'Content Team', 'Vincitore Bloccato', '~50', '$0 (contenuto)'],
        ['Template 3 livelli complaint\n(frustrazione/frode/minaccia)', 'CS Manager', 'Lamentatore Cronico', '~5', '$0 (template)'],
        ['Flag 14 utenti CRITICI nel CRM\ne assegnare priorita', 'CS Manager', 'Tutti i CRITICI', '~30', '$0 (CRM tag)'],
    ]
    story.append(make_table(w1, col_widths=[W*0.28, W*0.12, W*0.20, W*0.16, W*0.10], header_color=BRAND_GREEN))
    story.append(spacer(4))

    # Week 2
    story.append(Paragraph('SETTIMANA 2 (18-24 Marzo 2026) — Automazione', styles['H2']))
    w2 = [
        ['Azione', 'Chi', 'Persona Impattata', 'Ticket/Mese Risparmiati', 'Costo'],
        ['Widget: riconoscere "no funciona"\ne rispondere con troubleshooting', 'Dev Team', 'Tech-Frustrato', '~40', '$500-$1,000'],
        ['Widget: menu dopo "hola"\n(deposito/prelievo/bonus/tech)', 'Dev Team', 'Chattatore Seriale', '~100', '$300-$500'],
        ['Auto-merge ticket stesso\nutente entro 24h', 'Dev Team', 'Chattatore Seriale', '~80', '$500-$1,000'],
        ['KB: troubleshooting mobile\ncompleto (6 scenari)', 'Content Team', 'Tech-Frustrato', '~30', '$0 (contenuto)'],
        ['Outreach email ai 42 utenti\ncronici (5 mesi)', 'CS Manager', 'Tutti i cronici', '—', '$0 (email)'],
    ]
    story.append(make_table(w2, col_widths=[W*0.28, W*0.12, W*0.20, W*0.16, W*0.10], header_color=BRAND_AMBER))
    story.append(spacer(4))

    # Week 3-4
    story.append(Paragraph('SETTIMANE 3-4 (25 Mar - 7 Apr) — Integrazione GR8 API', styles['H2']))
    w34 = [
        ['Azione', 'Chi', 'Persona Impattata', 'Ticket/Mese Risparmiati', 'Costo'],
        ['Widget + GR8: status deposito\nreal-time (paymentTransactionV2)', 'Dev Team', 'Depositante Frustrato', '~120', '$2,000-$4,000'],
        ['Widget + GR8: check bonus/KYC\nquando utente chiede prelievo', 'Dev Team', 'Vincitore Bloccato', '~60', '$1,500-$3,000'],
        ['Pop-up pre-bonus con\ncalcolatrice rollover', 'Product Team', 'Giocatore Confuso', '~50', '$1,000-$2,000'],
        ['Auto-escalation dopo 2o\nticket stesso topic', 'Dev Team', 'Tutti i ricorrenti', '~40', '$500-$1,000'],
        ['Notifica push post-deposito\naccreditato', 'Dev Team', 'Depositante Frustrato', '~30', '$500-$1,000'],
    ]
    story.append(make_table(w34, col_widths=[W*0.28, W*0.12, W*0.20, W*0.16, W*0.10], header_color=BRAND_BLUE))
    story.append(spacer(6))

    # Total impact
    story.append(Paragraph('Impatto Totale dopo 4 Settimane:', styles['H2']))
    total_impact = [
        ['Metrica', 'Prima (Feb 2026)', 'Dopo (Apr 2026)', 'Miglioramento'],
        ['Ticket/mese totali', '~4,400', '~3,510', '-890/mese (-20%)'],
        ['Ticket/mese da ricorrenti', '~2,260', '~1,500', '-760/mese (-34%)'],
        ['AI resolution rate', '~42%', '~62%', '+20 punti percentuali'],
        ['First-contact resolution', '~35%', '~55%', '+20 punti percentuali'],
        ['Utenti frustrati/mese', '~300', '~150', '-50%'],
        ['Costo supporto/mese', '~$13,200', '~$10,530', '-$2,670/mese'],
        ['FTE risparmiati', '—', '~2.5 FTE', '+2.5 agenti per altre attivita'],
        ['Investimento totale', '—', '$6,800-$13,500', 'Una tantum'],
    ...
    doc.build(story)
    print('PDF generated: BetonWin_User_Recovery_Playbook.pdf')
